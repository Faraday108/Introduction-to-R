...
bad <- is.na(x)
x[!bad]
# What if there are multiple R objects and you want to take the subset with no missing values of those objects? 
x <- c(1, 2, NA, 4, NA, 5)
y <- c("a", "b", NA, "d", NA, "f")
...
Many operations in R are *vectorized*, meaning they occur in parallel in certain R objects allowing efficient, concise, easy to read code. 
Simple example of adding two vectors. 
```{r}
x <- 1:4
y <- 6:9
z <- x + y
...
for( i in seq_along(x)) {
 ...
 z
 ```
 Not as efficient! Another vectorized operation is logical comparisons. 
 ```{r}
 x <- 1:4
 x > 2
 x>=2
 x<3
 x == 1
 # Subtract, mult, divide are also vectorized
 ```
 
 ## Vectorized Matrix operations
 Enables element by element operations without having to loop over every element. 
```{r}
x <- matrix(1:4, 2, 2)
